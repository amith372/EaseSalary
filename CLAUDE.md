...
| the look of a screen — layout, palette, type, spacing, components | the design canvas, eleven artboards, the home screen in `EaseSalary - דף הבית v2.dc.html`: https://claude.ai/design/p/b11cf323-ac11-490d-994d-3145e8e07a6b — then Part 1 |
...
- The canvas is read for every screen but implemented one artboard at a time: the step names the artboard it builds, and the other ten are context rather than licence to build them — a session that implements an artboard its step did not name has skipped the check that step was going to end with (rule 7). A step that names none is reading only.
